...
export const canTechnicianUpdateReport = async (
  ...
  try {
    ...
    console.log("Maintainer categories:", maintainerCategoryId);
    ...
    if (maintainerCategoryId.includes(report.category_id)) {
      console.log(
        `Technician ${technician_id} is authorized to update report ${report_id}`
      );
      return true;
    } else {
      console.log(
        `Technician ${technician_id} is NOT authorized to update report ${report_id}`
      );
      return false;
    }
  } catch (error) {
