...
export default function Dashboard() {
  ...
  useEffect(() => {
    if (clickedReportId !== -1) {
      ...
      const report = reports.find((report) => report.id === clickedReportId);
      if (report) {
  ...
  useEffect(() => {
    const fetchReports = async () => {
      ...
      if (fetchedData.success) {
        const data = mapFetchedActiveReportsToActiveReports(fetchedData.data);
            ...
            {clickedReport && (
              <div className="flex flex-col pt-4 mb-3">
                ...
                <div className="flex flex-col overflow-scroll max-h-[50vh] pl-5 pr-5 pb-4 w-full">
                  ...
                  <SubTitle>
                    <div>{clickedReport.category}</div>
                  </SubTitle>
                  ...
                  <div className="flex flex-wrap gap-4">
                    {clickedReport.photos.map((photo) => (
                      <img className="h-50 w-50" src={photo} key={photo} />
                    ))}
                  </div>
